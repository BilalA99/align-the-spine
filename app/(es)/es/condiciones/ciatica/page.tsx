...
import { esSciatica } from "@/content/es/conditions";
...
const route = getEsRoute("/es/condiciones/ciatica");

export const metadata: Metadata = buildEsRouteMetadata(route);

/** /es/condiciones/ciatica — Spanish counterpart of /conditions/sciatica.
 *
 * Composed from the shared Spanish condition template
 * (components/sections/es-condition-page.tsx), driven by `esSciatica` in
 ...
 */
export default function EsSciaticaPage() {
  return (
    <>
      <JsonLd
        data={buildWebPage({
      ...
      />
      <EsConditionPage condition={esSciatica} />
    </>
  );
}
