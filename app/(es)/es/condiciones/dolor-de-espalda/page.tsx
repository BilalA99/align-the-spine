...
import { esBackPain } from "@/content/es/conditions";
...
const route = getEsRoute("/es/condiciones/dolor-de-espalda");

export const metadata: Metadata = buildEsRouteMetadata(route);

/** /es/condiciones/dolor-de-espalda — Spanish counterpart of /conditions/back-pain.
 *
 * Composed from the shared Spanish condition template
 * (components/sections/es-condition-page.tsx), driven by `esBackPain` in
 ...
 */
export default function EsBackPainPage() {
  return (
    <>
      <JsonLd
        data={buildWebPage({
      ...
      />
      <EsConditionPage condition={esBackPain} />
    </>
  );
}
